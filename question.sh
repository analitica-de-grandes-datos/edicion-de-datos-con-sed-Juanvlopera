...
# 
sed 's/;C;\\N;/;C;\\N;\\N/g' data.csv | sed 's/\([0-9]\)\/\([0-9]\)\/\([0-9]*\);/\3-0\2-0\1;/g' | sed 's/\([0-9]*\)\/\([0-9]*\)\/\([0-9][0-9]\);/20\3-\2-\1;/g' | sed 's/.*/\U&/g' | sed 's/;N/;\\\N/g' | sed 's/;;/;\\\N;/g ' | sed 's/;\\N;\\N;/;\\N;\\N;\\N/g' | sed 's/,/./g' | sed 's/;/,/g' 